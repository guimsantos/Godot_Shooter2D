...
var react_time = rand_range(200, 401)
var dir = 0
var next_dir = 0
var next_dir_time = 0
...
func _process(delta):
	...
	if OS.get_ticks_msec() > next_jump_time and next_jump_time != -1 and is_on_floor():
		if Player.position.y < position.y - 24 and sees_player():
			vel.y = -450
		next_jump_time = -1

	vel.x = dir * 180

	if Player.position.y < position.y - 24 and next_jump_time == -1 and sees_player():
